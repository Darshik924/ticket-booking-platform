...
const worker = new Worker("paymentQueue", async job => {
  ...
  console.log(`[PaymentWorker] Processing payment for job ${job.id}: user ${userId}, seat ${seatId}`);
  ...
  try {
    ...
    console.log(`[PaymentWorker] Payment successfully processed for booking ${bookingId}`);
  } catch (error) {
    console.error(`[PaymentWorker] Failed to process payment for booking ${bookingId}:`, error);
    ...
    try {
      ...
      if (seatVal) {
        const colonIdx = seatVal.indexOf(":");
        const seatNumber = seatVal.substring(0, colonIdx);
        await redisClient.hset(hashKey, String(seatId), `${seatNumber}:AVAILABLE`);
      }
      ...
      console.error("[PaymentWorker] Critical error during payment error cleanup:", cleanupErr);
    }
  }
}, {
...
console.log("[PaymentWorker] Background worker started listening to paymentQueue");
